...
if __name__ == "__main__":
    ...
    for i in range(conf.n):
        ...
        data, tree = generate_model(conf.tree_size, conf.bins, conf.cells, MODEL_ID, high_noise=conf.high_noise,
                                    out_dir=dirpath)
        ...
        FP = 0.0
        FN = 0.0
        SymmetricDifference = 0.0
        real_bp_count = 0.0
        inferred_bp_count = 0.0

        for cell in range(0, inferred_cn.shape[1]):
            for bin in range(1, inferred_cn.shape[0]):
                if inferred_cn[bin, cell] != inferred_cn[bin - 1, cell]:
                    inferred_bp_count += 1.0

                if real_cn.T[bin, cell] != real_cn.T[bin -1, cell]:
                    real_bp_count += 1.0

                if inferred_cn[bin, cell] != inferred_cn[bin -1, cell] and real_cn.T[bin, cell] == real_cn.T[bin -1, cell]:
                    SymmetricDifference += 1.0
                    FP += 1.0
                elif inferred_cn[bin, cell] == inferred_cn[bin -1, cell] and real_cn.T[bin, cell] != real_cn.T[bin -1, cell]:
                    SymmetricDifference += 1.0
                    FN += 1.0
        ...
        statistics.append(
            Statistics(
                ...
                node_sensitivity=sensitivity(inferred_nodes, real_nodes),
                FP=FP / inferred_bp_count if inferred_bp_count > 0.0 else 0.0 ,
                FN=FN / real_bp_count if real_bp_count > 0.0 else 0.0,
                SymmetricDifference=SymmetricDifference / float(conf.cells)
            ).dict()
        )
