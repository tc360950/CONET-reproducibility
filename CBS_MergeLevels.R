...
X <- read.table(opt$dataset, sep=";", header=F)
X <- X[2:dim(X)[1],]
colnames(X)[c(1,2,3,4,5)] <- c("chr", "start", "end", "width", "candidate_brkp")
X_segmented <- X

print(colnames(X_segmented))
...
write.table(X, file=opt$output, sep=",", row.names=F, quote=FALSE, col.names=F)